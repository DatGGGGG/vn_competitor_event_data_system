...
VN_EVENT_DW_API_KEYS_FILE=/app/data/api_keys.json
```
...
sudo docker compose --env-file deploy/docker/vm.env -f deploy/docker/docker-compose.yml --profile ops run --rm --no-deps job \
  vn-event-dw api-key-generate --keys-file /app/data/api_keys.json --name teammate-name
```

List or revoke managed keys:

```bash
sudo docker compose --env-file deploy/docker/vm.env -f deploy/docker/docker-compose.yml --profile ops run --rm --no-deps job \
  vn-event-dw api-key-list --keys-file /app/data/api_keys.json

sudo docker compose --env-file deploy/docker/vm.env -f deploy/docker/docker-compose.yml --profile ops run --rm --no-deps job \
  vn-event-dw api-key-revoke --keys-file /app/data/api_keys.json --name teammate-name
```

After `VN_EVENT_DW_API_KEYS_FILE` is added to `deploy/docker/pipeline.env`, recreate the API container once. Generating or revoking keys after that does not require a restart because the API reads the key file on each request.
